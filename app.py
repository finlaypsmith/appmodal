...
UUID = os.environ.get('UUID', '{{UUID}}')       # UUID
NEZHA_SERVER = os.environ.get('NEZHA_SERVER', '{{NEZHA_SERVER}}')                  # 哪吒3个变量不全不运行
NEZHA_PORT = os.environ.get('NEZHA_PORT', '{{NEZHA_PORT}}')                           # 哪吒端口为{443,8443,2096,2097,2083}其中之一时自动开启tls
NEZHA_KEY = os.environ.get('NEZHA_KEY', '{{NEZHA_KEY}}')                                 # 哪吒客户端密钥
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', '{{ARGO_DOMAIN}}')                             # 国定隧道域名，留空即启用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', '{{ARGO_AUTH}}')                                 # 国定隧道json或token，留空即启用临时隧道,json获取地址：https://fscarmen.cloudflare.now.cc
ARGO_PORT = int(os.environ.get('ARGO_PORT', 8001))                          # Argo端口，固定隧道token请改回8080或在cf后台设置的端口与这里对应
CFIP = os.environ.get('CFIP', '{{CFIP}}')                            # 优选域名或优选ip
CFPORT = int(os.environ.get('CFPORT', 443))                                 # 优选域名或优选ip对应端口
NAME = os.environ.get('NAME', '{{NAME}}')                                        # 节点名称
